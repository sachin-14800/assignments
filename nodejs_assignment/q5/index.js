...
const csvWriter=createCsvWriter({
    path:'urls.csv',
    header:[{id:'original',title:'Original'},{id:'short',title:'Shortened'}]
});

ShortUrl=(arr)=>{
    arr.forEach( (element)=>{
        urlShortner.short(element,async(err,url)=>{
            if(err){console.log(err);}
            await csvWriter.writeRecords([{original:element,short:url}]);
        });
